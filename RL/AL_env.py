...
class AL_env(object):
    ...
    def __init__(self, args, decoder, optimizer, scheduler, learning_assess_data, simulator, log_prior, logger, save_folder, valid_data_loader, edge, mins, maxs, discrete_mapping=None, lstm_direction=2):
        self.edge = edge
        self.mins = mins
        self.maxs = maxs
        self.epoch = 0
        self.feature_dims = args.extract_feat_dim
        ...
        self.obj_extractor = MLPEncoder(args, 3, 128, self.feature_dims).cuda()
        ...
        self.obj_num = args.initial_obj_num
    ...
    def action_to_new_data(self, action):
        ...
        return trajectory, query_setting

    def intervene_graph(self, action, new_datapoint):
        """[summary]

        Args:
            action ([type]): [description]
            new_datapoint ([type]): [description]
        """
        ...
        for l in self.obj_data.values():
            if l:
                for d in l:
                    ...
                    new_setting.requires_grad = True
                    setting.requires_grad = True
                    causal = (new_setting-setting).unsqueeze(0)
                    caused = abs(
                        new_datapoint[0, :, -1, 0]-d[0, :, -1, 0]).unsqueeze(1)
                    relations = (causal*caused).flatten().cuda()
                    relations = torch.cat(
                        [relations.unsqueeze(-1)]*2, axis=1)
                    # import pdb
                    # pdb.set_trace()
                    # with torch.no_grad():
    ...
    def step(self, args, idx, new_datapoint):  # :action, memory):
        """[summary]
        ...
        done = len(self.train_dataset) > self.budget
        self.epoch += 1
