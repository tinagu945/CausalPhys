...
train_loader, valid_loader, test_loader = load_my_data(args.batch_size)
...
def train(epoch, best_val_accuracy):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # Flatten ba = output.view(-1, args.num_classes)
        ...
        correct = torch.nonzero(target.data.view_as(pred)+pred==2).size(0)
        acc = correct / torch.nonzero(target).size(0)#pred.size(0)
    ...
    for batch_idx, (data, target) in enumerate(valid_loader):
        ...
        loss = F.cross_entropy(output, target)
        pred = output.data.max(1, keepdim=True)[1]
        print(pred[:10],target.data.view_as(pred)[:10])
        #correct = pred.eq(target.data.view_as(pred)).sum().item()
        correct = torch.nonzero(target.data.view_as(pred)+pred==2).size(0)
...
def test():
    model_file = os.path.join('logs/exp26', 'encoder.pt')
    t = time.time()
    loss_test = []
    acc_test = []
    model.eval()
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        pred = output.data.max(1, keepdim=True)[1]
        print(torch.nonzero(pred), torch.nonzero(target))                       
        #correct = pred.eq(target.data.view_as(pred)).sum().item()
        correct = torch.nonzero(target.data.view_as(pred)+pred==2).size(0)
        acc = correct / torch.nonzero(target).size(0)#pred.size(0)
