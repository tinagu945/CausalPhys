...
parser.add_argument('--sample-k', type=int, default=1,
                    help='Sample the top k nodes that satisfy the sampling rule')
parser.add_argument('--sample-num-batches', type=int, default=1,
                    help='For each sampling node, sample #batches of it.')
parser.add_argument('--log_epoch', type=int, default=5,
                    help='Interval between each train_log.')
...
parser.add_argument('--sampler', type=str, default='random',
                    help='Which sampler to use. Currently only support random and uncertainty')

args = parser.parse_args()
args.num_atoms = args.input_atoms+args.target_atoms
# AL must use controlled dataset
args.grouped = True
args.script = 'train_causal_grouped_AL_dataset'
print(args)
...
def main():
    ...
    if args.sampler == 'random':
        sampler = RandomDatasetSampler(train_data)
    elif args.sampler == 'uncertainty':
        sampler = MaximalEntropyDatasetSampler(train_data)
    else:
        ...
        if epoch == 0:
            ...
            for i in range(args.input_atoms):
                new_data_idx = sampler.sample(
                    [i]*args.sample_k, args.variations, num_batches=args.sample_num_batches)
                uncertain_nodes = torch.LongTensor(
                    [i]).expand(new_data_idx.size()).cuda()
                data_idx.append(new_data_idx)
                nodes.append(uncertain_nodes)

            data_idx = torch.cat(data_idx)
            nodes = torch.cat(nodes)
            train_dataset = ALIndexDataset(train_data, data_idx, nodes)
        else:
            control_nodes = sampler.criterion(
                decoder.rel_graph, k=args.sample_k)
            new_data_idx = sampler.sample(
                control_nodes, args.variations, num_batches=args.sample_num_batches)
            uncertain_nodes = torch.LongTensor(
                control_nodes).repeat_interleave(new_data_idx.size(0)//args.sample_k).cuda()
            train_dataset = update_ALIndexDataset(
                train_data, train_dataset, new_data_idx, uncertain_nodes)
            with open(os.path.join(save_folder, str(epoch)+'_nodes.pt'), 'wb') as f:
                torch.save(
                    train_dataset.nodes[0::args.variations*args.sample_num_batches], f)
                print('sampled ndoes',
                      train_dataset.nodes[0::args.variations*args.sample_num_batches])
        train_data_loader = DataLoader(
            train_dataset, batch_size=args.variations, shuffle=False)
#         print(train_dataset.data[:,:,:3,0], train_dataset.nodes)
        ...
        train_control(args, log_prior, logger, optimizer, save_folder,
                      train_data_loader, epoch, decoder, rel_rec, rel_send, log_epoch=args.log_epoch)
