...
def generate_dataset_discrete_spaced(scenario, lows, highs, num_variations, candidate_multiples):
    assert len(lows) == len(
        highs) and scenario.num_inputs == len(lows)
    values = []
    for i in range(scenario.num_inputs):
        candidates = np.linspace(
            lows[i], highs[i], num_variations*candidate_multiples)
        np.random.shuffle(candidates)
        values.append(candidates[:num_variations].tolist())
...
def main():
    ...
    #     SHO_scenario, SHO_lows, SHO_highs, train_variations, train_multiples)
    # SHO_valid_discrete_spaced_interpolation = generate_dataset_discrete_spaced(
    #     SHO_scenario, SHO_lows, SHO_highs, valid_variations, valid_multiples)
    ...
    #         'train_causal_vel_SHO_spaced_nowall_50_new')
    # save_ds(SHO_train_cont, SHO_edge,
    #         'train_causal_vel_SHO_cont_nowall_50_new')
    # save_ds(SHO_valid_discrete_spaced_interpolation, SHO_edge,
    #         'valid_causal_vel_SHO_spaced_nowall_50_new_interpolation')
    # save_ds(SHO_valid_interpolation, SHO_edge,
    #         'valid_causal_vel_SHO_cont_nowall_50_new_interpolation')
    # print('SHO finished.')

    airfall_lows = np.zeros(6)+0.2
    ...
    airfall_train_discrete_spaced = generate_dataset_discrete_spaced(
        airfall_scenario, airfall_lows, airfall_highs, train_variations, train_multiples)
    airfall_valid_discrete_spaced_interpolation = generate_dataset_discrete_spaced(
        airfall_scenario, airfall_lows, airfall_highs, valid_variations, valid_multiples)
    ...
    save_ds(airfall_valid_discrete_spaced_interpolation, airfall_edge,
            'valid_causal_vel_airfall_spaced_onestep_c_new_interpolation')
    save_ds(airfall_valid_interpolation, airfall_edge,
            'valid_causal_vel_airfall_onestep_onestep_c_new_interpolation')
    ...
    #     slide_scenario, slide_lows, slide_highs, train_variations, train_multiples)
    # slide_valid_discrete_spaced_interpolation = generate_dataset_discrete_spaced(
    #     slide_scenario, slide_lows, slide_highs, valid_variations, valid_multiples)
