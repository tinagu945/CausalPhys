...
def train_val_control(args, log_prior, logger, optimizer, save_folder, train_loader, valid_data_loader,
                      decoder, rel_rec, rel_send, data_trained, mask_grad=False, train_log=5, val_log=1, dataset_size=46656):
    """
...
#             decoder.rel_graph.grad *= mask
        ...
        if data_trained % train_log == 0:
            print('Train logging...')
            logger.log('train', decoder, data_trained//dataset_size, np.mean(nll_train), np.mean(nll_train_lasttwo), kl=np.mean(kl_train), mse=np.mean(mse_train), a=np.mean(a_train), b=np.mean(b_train), c=np.mean(
        ...
        if data_trained % val_log == 0:
            val_control(
                args, log_prior, logger, save_folder, valid_data_loader, data_trained//dataset_size, decoder, rel_rec, rel_send)

    print('Train AVG this epoch', np.mean(control_train),
          np.mean(kl_train), np.mean(nll_train), np.mean(nll_train_lasttwo))
    print(data_trained//dataset_size, decoder.rel_graph.softmax(-1),
